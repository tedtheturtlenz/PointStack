...
def validate(net, testloader, criterion, device, is_segmentation = False):
    ...
    with torch.no_grad():
        for batch_idx, data_dic in enumerate(tqdm(testloader)):
            ...
            if is_segmentation:
                miou = net.compute_overall_iou(data_dic['pred_score_logits'], data_dic['seg_id'], num_classes = 3)
                ...
                seg_pred = data_dic['pred_score_logits'].contiguous().view(-1, 3) # Bonsai Has three classes
